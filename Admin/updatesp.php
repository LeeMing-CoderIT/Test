...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <h1 class="page-head-line">Thay đổi thông tin sản phẩm</h1>
                <div class="row">
                    <div class="col-md-12">
                        <div class="panel panel-default">
                            <?php
                                $id=0;
                                if(isset($_GET['idsp']))    $id = $_GET['idsp'];
                            ...
                            <form method="post" enctype="multipart/form-data" style=" margin-top: 10px; margin-left: 10px;">
                                ...
                                <span style="font-size: 20px;">Tên sản phẩm: </span>
                                    <input type="text" name="txttensp" value="<?php echo$sp["Name_SP"]; ?>"><br><br>
                                <span style="font-size: 20px;">Giá sản phẩm:  </span>
                                    <input type="text" name="txtgiasp" value="<?php echo$sp["Price"]; ?>"><br><br>
                                <span style="font-size: 20px;">Số lượng tồn kho:  </span>
                                    <input type="number" name="txtslsp" value="<?php echo$sp["SLConLai"]; ?>"><br><br>
                                <span style="font-size: 20px;">Mô tả:  </span>
                                    <input type="text" style="height: 50px; width: 400px;" name="txtmota" value="<?php echo$sp["MoTa"]; ?>"><br><br>
                                <span style="font-size: 20px;">Ảnh của sản phẩm: </span>
                                    <img src="../User/images/image_SP/<?php echo$sp["Image_SP"]; ?>" style="width: 50px; height: 50px;" alt="<?php echo$sp["Image_SP"]; ?>">
                            ...
                            <?php
                                if(isset($_POST['oke'])){
                                    $file = $sp["Image_SP"];
                                    if(!empty($_FILES['img_sp']['tmp_name'])){
                                        move_uploaded_file($_FILES['img_sp']['tmp_name'],'../User/images/image_SP/'.$_FILES['img_sp']['name']);
                                        $file = $_FILES['img_sp']['name'];
                                    }
                                    $upsp = $getdata->Update_SP($sp["ID_SP"],$_POST['txttensp'],$_POST['txtgiasp'],$_POST['txtslsp'],$_POST['txtmota'],$file);
                                    if($upsp){
                                        echo"<script>
